 ...
 "cells": [
  {
   ...
   "source": [
    ...
    "\n",
    "### ⚡ Sélection automatique du Matériel (GPU vs CPU)\n",
    "- **Si n < 100 000** : Le mode **CPU** est sélectionné automatiquement car il est plus rapide (~10 s par échantillon de 1000 étapes) en évitant le coût de lancement des noyaux GPU.\n",
    "- **Si n >= 100 000** : Le mode **GPU** (si disponible) est sélectionné automatiquement."
   ]
  },
  {
   "cell_type": "code",
   ...
   "source": [
    ...
    "# ==========================================\n",
    "# Si n < 100 000, le CPU est plus rapide en raison du coût de lancement CUDA.\n",
    "# Si n >= 100 000 et qu'un GPU est disponible, on choisit automatiquement le GPU.\n",
    "use_gpu = has_gpu and (n >= 100000)\n",
    "\n",
  ...
  },
  {
   ...
   "source": [
    ...
    "std_lcc_gpu = {m: xp.std(overlap_history[m], axis=0).get() if use_gpu and has_gpu else xp.std(lcc_history[m], axis=0) for m in modes}\n",
    "\n",
