 ...
 "cells": [
  ...
  },
  {
   ...
   "source": [
    ...
    "\n",
    "    # 100% GPU - CSR Matrix ultra-optimisée pour éviter adj + adj.T\n",
    "    row = edges[frozen, 0]\n",
    "    col = edges[frozen, 1]\n",
    "    N_nodes = len(sigma)\n",
    "    \n",
    "    # Symétrisation manuelle + auto-boucles pour éviter le tri lourd et l'addition creuse\n",
    "    sym_row = cp.concatenate((row, col, cp.arange(N_nodes))).astype(cp.int32)\n",
    "    sym_col = cp.concatenate((col, row, cp.arange(N_nodes))).astype(cp.int32)\n",
