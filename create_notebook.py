 ...
 "cells": [
  {
   ...
   "metadata": {},
   "source": [
    "# Comparaison des dynamiques Swendsen-Wang (Edges vs Triangles) avec CuPy\n",
    "\n",
  ...
  },
  {
   ...
   "source": [
    ...
    "        if len(B_idx) > 0:\n",
    "            sat_B = sat_tri[B_idx]\n",
    "            options = sat_B * cp.array([1, 2, 3])\n",
    "            options.sort(axis=1) # repousse les 0 au début, les index valides aux positions 1 et 2\n",
    "            pick = cp.random.randint(1, 3, size=len(B_idx))\n",
    "            chosen_col = options[cp.arange(len(B_idx)), pick] - 1\n",
    ...
    "    \n",
    "    lcc_frac = float(cp.max(cp.bincount(labels))) / len(sigma)\n",
    "    \n",
    "    flip = cp.random.choice(cp.array([-1, 1]), size=n_comp)\n",
    "    sigma = sigma * flip[labels]\n",
    "    \n",
    "    return sigma, float(lcc_frac)"
   ]
  },
  {
   "cell_type": "code",
   ...
   "source": [
    ...
    "    # Vérité terrain (Σ)\n",
    "    Sigma = cp.random.choice(cp.array([-1, 1]), size=L*L)\n",
    "    \n",
    "    # Observations W conditionnées sur Σ\n",
    "    same_comm = Sigma[edges[:,0]] == Sigma[edges[:,1]]\n",
    "    correct_obs = cp.random.rand(len(edges)) < p\n",
