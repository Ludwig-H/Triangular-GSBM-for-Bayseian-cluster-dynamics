 ...
 "cells": [
  ...
  },
  {
   ...
   "source": [
    ...
    "\n",
    "# Choix du backend pour la recherche de composantes connexes ('gpu' ou 'cpu')\n",
    "# En Google Colab, 'gpu' est BEAUCOUP plus rapide car il évite la latence PCIe de la machine virtuelle.\n",
    "cc_backend = 'gpu'\n",
    "\n",
    "# Paramètres\n",
    "d = 2\n",
  ...
  },
  {
   ...
   "outputs": [],
   "source": [
    "def SW_step(sigma, W, edges, mode, up, triangles, type_A, au, cc_backend='gpu', edges_cpu=None):\n",
    "    N_edges = len(edges)\n",
    ...
    "    N_nodes = len(sigma)\n",
    "    if cc_backend == 'gpu':\n",
    "        # Calculs 100% GPU (recommandé sur Google Colab pour éviter la latence PCIe de la VM)\n",
    "        frozen_edges = edges[frozen]\n",
    "        row = frozen_edges[:, 0]\n",
    "        col = frozen_edges[:, 1]\n",
    "        data = cp.ones_like(row, dtype=cp.float32)\n",
    "        adj = csr_matrix((data, (row, col)), shape=(N_nodes, N_nodes))\n",
    "        n_comp, labels = connected_components(adj, directed=False)\n",
    "    else:\n",
    "        # Calcul hybride GPU-CPU (optimal en local avec latence PCIe ultra-faible)\n",
    "        if edges_cpu is not None:\n",
    "            # Utilisation de la copie CPU pré-allouée : évite la double-synchronisation du masque dynamique\n",
    "            frozen_edges = edges_cpu[frozen.get()]\n",
    "        else:\n",
    "            frozen_edges = edges[frozen].get()\n",
    "        row = frozen_edges[:, 0]\n",
    "        col = frozen_edges[:, 1]\n",
    "        data = np.ones(len(row), dtype=np.float32)\n",
    "        \n",
    "        adj_cpu = scipy.sparse.csr_matrix((data, (row, col)), shape=(N_nodes, N_nodes))\n",
    "        n_comp, labels_cpu = scipy.sparse.csgraph.connected_components(adj_cpu, directed=False)\n",
  ...
  },
  {
   ...
   "source": [
    ...
    "        lcc_list = []\n",
    "        for t in range(T):\n",
    "            sigma, lcc = SW_step(sigma, W, edges_gpu, mode, up, triangles, type_A, au, cc_backend=cc_backend, edges_cpu=edges_cpu)\n",
    "            ov = cp.abs(cp.mean(sigma * Sigma))\n",
    "            ov_list.append(ov)\n",
    "            lcc_list.append(lcc)\n",
    "            \n",
    "        # Affectation unique par ligne (élimine 2000 écritures par élément sur GPU)\n",
    "        overlap_history[mode][sample] = cp.stack(ov_list)\n",
    "        lcc_history[mode][sample] = cp.stack(lcc_list)"
   ]
  },
  {
   "cell_type": "code",
