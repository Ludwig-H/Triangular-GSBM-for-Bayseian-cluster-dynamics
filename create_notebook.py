 ...
 "cells": [
  {
   ...
   "source": [
    ...
    "\n",
    "### ⚡ Choix du Matériel (GPU vs CPU)\n",
    "- **Pour N = 10 000 (défaut)** : Le mode **CPU** est beaucoup plus rapide (~10 s par échantillon de 1000 étapes) car il évite le coût de lancement des noyaux GPU.\n",
    "- **Pour N >= 100 000** : Le mode **GPU** devient beaucoup plus rapide car la taille des données sature les processeurs de la carte graphique."
   ]
  },
  {
   "cell_type": "code",
   ...
   "outputs": [],
   "source": [
    "# Tentative d'import de CuPy\n",
    "try:\n",
    "    import cupy as cp\n",
    "    has_gpu = True\n",
    "except ImportError:\n",
    "    has_gpu = False\n",
    "    print(\"CuPy n'est pas installé. Exécution possible uniquement sur CPU.\")\n",
    "\n",
    "import numpy as np\n",
    ...
    "\n",
    "# ==========================================\n",
    "# CONFIGURATION DE L'EXÉCUTION (GPU / CPU)\n",
    "# ==========================================\n",
    "# Activez le GPU uniquement pour les grands graphes (N >= 100 000).\n",
    "# Pour N = 10 000, laissez `use_gpu = False` pour obtenir les meilleurs temps d'exécution (~10s par échantillon).\n",
    "use_gpu = False  # True pour utiliser le GPU (avec CuPy), False pour utiliser le CPU (avec NumPy)\n",
    "\n",
    ...
    "    xp = cp\n",
    "    print(\"Exécution configurée sur : GPU (CuPy)\")\n",
    "else:\n",
    "    import scipy.sparse\n",
    ...
    "    xp = np\n",
    "    print(\"Exécution configurée sur : CPU (NumPy/SciPy)\")\n",
    "\n",
    "# Paramètres\n",
    "d = 2\n",
    "p = 0.65\n",
  ...
  },
  {
   ...
   "source": [
    ...
    "        S0 = sat_tri[:, 0]\n",
    "        S2 = sat_tri[:, 2]\n",
    "        C1 = 1 - S0.astype(cp.int32) if use_gpu and has_gpu else 1 - S0.astype(np.int32)\n",
    "        C2 = 1 + S2.astype(cp.int32) if use_gpu and has_gpu else 1 + S2.astype(np.int32)\n",
    "        \n",
